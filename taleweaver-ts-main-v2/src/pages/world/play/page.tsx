...
export default function World() {
  ...
  useEffect(() => {
    if (pathname) {
      ...
      setThread(thread);
      getDecisions({ thread, run }).then(async (response: GetDecisionsResponseProps) => {
        console.log("opeai response", response)
        let { context, consequence, previousDecision, decisions, dallePrompt,
          storyContext} = response;
        console.log(response)
        setContextSummary(previousDecision);
        context = context || storyContext || consequence;
        setContext(context);
        setDecisions(decisions.map((d: any) => { return { ...d, value: d.id } }));
    ...
    <Form noValidate validated={false} onSubmit={onPlay} id="play-form">
      <div className="container">
        <div style={{ height: "35rem" }}>
          <CoverImage id={thread} src={image} alt={world.title} title={contextSummary}
            playButton={true} />
        </div>
        <p className="py-2">
          {context || <Skeleton count={5} />}
        </p>
